...
class Kasir extends CI_Controller
{
	...
	public function nomor_faktur()
	{
		...
		if ($user == $nm_user && $selesai == '0') {
			...
			$nofaktur = $this->kasir_model->getNo($id_user, $nm_user);
			$data = array(
		...
		redirect('kasir/form-pesan/' . $nofaktur, 'refresh');
	}
	...
	public function form_pesan()
	{
		...
		$list_sales = $this->kasir_model->listsales($noresi)->row();
		...
		if ($data_faktur) {
			...
			$this->load->view('kasir/form_pesan', $data);
	...
	public function antrian_pesan()
	{
		...
		$data['pending'] = $this->kasir_model->antrianpesan($id_user, $now, $before);
		$data['no'] = 1;
		...
		$setting['seting'] = $this->login_model->seting()->row();
		$this->load->view('header', $setting);
		$this->load->view('kasir/antrian_pesan', $data);
	...
	{
		if (isset($_GET['term'])) {
			$result = $this->kasir_model->cari_cust($_GET['term']);
			if (count($result) > 0) {
	...
	{
		if (isset($_GET['term'])) {
			$result = $this->kasir_model->cari_sales($_GET['term']);
			if (count($result) > 0) {
				foreach ($result as $row) {
					$arr_result[] = array(
						'label' => $row->NmSales,
	...
	{
		if (isset($_GET['term'])) {
			$result = $this->kasir_model->cari_brg($_GET['term']);
			if (count($result) > 0) {
				foreach ($result as $row) {
					$arr_result[] = array(
						'kode' => $row->KdBrg,
	...
	public function go_to_simpan()
	{
		...
		$nofaktur = $this->kasir_model->getNoFaktur($ymd, $id_user, $nm_user);
		$cek = $this->kasir_model->cek_go_simpan($noresi, $nm_user);
		$urix = base_url('kasir/form_pesan/') . $noresi;
		$uri = base_url('kasir/nomor-faktur/');
		$x = $cek->row_array();
		$cust = $x['KdCust'];
	...
	{
		$id_user = $this->session->userdata('ses_username');
		$data['selesai'] = $this->kasir_model->pesanpending($id_user);
		$data['no'] = 1;
		...
		$setting['seting'] = $this->login_model->seting()->row();
		$this->load->view('header', $setting);
		$this->load->view('kasir/pesan_pending', $data);
		$this->load->view('footers');
	}
	...
	public function lap_jual_konter()
	{
		$tgl_now = date('Y-m-d');
		$id_user = $this->session->userdata('ses_username');
		$data['selesai'] = $this->kasir_model->lapjualkonter($id_user, $tgl_now);
		$data['no'] = 1;
		...
		$setting['seting'] = $this->login_model->seting()->row();
		$this->load->view('header', $setting);
		$this->load->view('kasir/lap_jual_konter', $data);
		$this->load->view('footer');
	}
	...
	public function kirimprint()
	{
		...
		//$data['pj'] = $this->kasir_model->cek_nopj($nofaktur);
		$cekpesanjual = $this->kasir_model->cek_nopj($nofaktur);
		$pj = $cekpesanjual->row_array();
		$telp = $pj['Telp'];
		$nmcust = $pj['NmCust'];
		$total = $pj['SubTotal'];
		$uri = base_url('kasir/pesan_pending/');
	...
	public function cekcust()
	{
		...
		$customer = $this->kasir_model->getcustomer($idcust);
		$cek_sudah_ada = $this->kasir_model->cek_cust_ada($idcust, $nofaktur);
		$x = $customer->row_array();
		$kdcust = $x['KdCust'];
		$nmcust = $x['NmCust'];
		$harga = $x['HrgJual'];
		$uri = base_url('kasir/form-pesan/') . $nofaktur;
	...
	public function ceksales()
	{
		...
		$sales = $this->kasir_model->getsales($idsales);
		$cek_sudah_ada = $this->kasir_model->cek_sales_ada($idsales, $nofaktur);
		$x = $sales->row_array();
		$kdsales = $x['KdSales'];
		//$nmsales = $x['NmSales'];
		$uri = base_url('kasir/form-pesan/') . $nofaktur;
	...
	public function edit_satuan()
	{
		...
		$cek_satuan_ada = $this->kasir_model->cek_satuan_ada($idbarang, $nofaktur);
	...
	function edit_jenis_harga()
	{
		...
		$cek_jenishrg = $this->kasir_model->cek_jenis_harga($nofaktur);
		//$cek_tpesan = $this->kasir_model->cek_tpesan($nofaktur);
	...
	public function cekbarang()
	{
		...
		$cek_jenishrg = $this->kasir_model->cek_jenis_harga($nofaktur);
		$lokasi = $this->kasir_model->cek_lokasi($nofaktur)->row_array();
		$lok = $lokasi['KdLokasi'];

		$stock = $this->kasir_model->stocklokasi($idbarang, $lok);
		$stk = $stock->row_array();
		$x = $produk->row_array();
		$kdb = $x['KdBrg'];
		$h1 = $x['HrgJl11'];
		...
		$diskonrp = "0";
		$diskonpersen = "0";
		$cek_sudah_ada = $this->kasir_model->cek_sudah_ada($kdb, $nofaktur);
		$cek_tpesan = $this->kasir_model->cek_tpesan($nofaktur);
	...
	public function edit_kdsales()
	{
		...
		$ceksales = $this->kasir_model->ceksales_detail($idsales, $idbarang, $nofaktur);
		$getsales = $this->kasir_model->getsales($idsales);
	...
	public function hapus_faktur()
	{
		...
		redirect('kasir/nomor-faktur/', 'refresh');
	}
