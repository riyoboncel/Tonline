...
class Cafe extends CI_Controller
{
    ...
    public function nomor_faktur()
    {
        ...
        if ($user == $nm_user && $selesai == '0') {
            ...
            $nofaktur = $this->cafe_model->getNo($id_user, $nm_user);
            $data = array(
        ...
        redirect('cafe/form-cafe/' . $nofaktur, 'refresh');
    }
    ...
    public function form_cafe()
    {
        ...
        $data_faktur = $this->cafe_model->getDataPenjualan($noresi, $username)->row();
        $list_barang = $this->cafe_model->getListPenjualan($noresi);
        $list_sales = $this->cafe_model->listsales($noresi)->row();
        ...
        if ($data_faktur) {
            ...
            $data['tot_item'] = 0;
            $data['tot_belanja'] = 0;
            $data['belanja'] = $this->cafe_model->getTotalBelanja($noresi)->row();
            ...
            $this->load->view('cafe/form_cafe', $data);
    ...
    public function cari_item()
    {
        ...
        $data['title'] = 'Entry Pesan Cafe';
        $data['kategori'] = $this->cafe_model->departemen();

        $setting['user'] = $this->login_model->sistemuser($username)->row();
		$setting['seting'] = $this->login_model->seting()->row();
        
        $data['faktur'] = $this->cafe_model->getDataPenjualan($noresi, $username)->row();
        if ($departemen <> '') {
            $data['item'] = $this->cafe_model->getItems($departemen);
        } else {
            $data['item'] = $this->cafe_model->getItem();
        }

        $this->load->view('header', $setting);
        $this->load->view('cafe/cari_item', $data);
        $this->load->view('footers');
    }
    ...
    public function kirim_barang()
    {
        ...
        $produk = $this->cafe_model->getbarang($idbarang);
        $cek_jenishrg = $this->cafe_model->cek_jenis_harga($nofaktur);
        ...
        //$h4 = $x['HrgJl14'];
        $jumlah = "1";
        $cek_sudah_ada = $this->cafe_model->cek_sudah_ada($kdb, $nofaktur);
        //$cek_tpesan = $this->Cafe_model->cek_tpesan($nofaktur);


        $uri = base_url('cafe/form-cafe/') . $nofaktur;
        ...
        if ($produk->num_rows() > 0) {
            ...
            if ($cek_sudah_ada->num_rows() > 0) {
                ...
                if ($kode <> 1) {
                    ...
                    echo $this->session->set_flashdata('error', 'Kode ' . $idbarang . 'jumlah beli sekarang ' . $jum_beli_sekarang . ' database ' . $jum_beli . ' sudah ada :(');
                    header("Location: " . $uri, TRUE, $http_response_code);
                }
            } else {
                $input = array(
    ...
    public function json_produk()
    {
        if ($this->input->is_ajax_request()) {
            $this->cafe_model->getProduk();
        } else {
            redirect('cafe/cari_item_datatables', 'refresh');
        }
    }


    // tampilan pesan yang belum di selesaikan 
    public function antrian_cafe()
    {
        $id_user = $this->session->userdata('ses_username');
        $username = $this->session->userdata('ses_username');
        $now = date('Y-m-d');
        $before = date('Y-m-d', strtotime('-30 days', strtotime($now)));
        $data['pending'] = $this->cafe_model->antrianpra($id_user, $now, $before);
        $data['no'] = 1;

        $setting['user'] = $this->login_model->sistemuser($username)->row();
        $setting['seting'] = $this->login_model->seting()->row();
        $this->load->view('header', $setting);
        $this->load->view('cafe/antrian_cafe', $data);
        $this->load->view('footers');
    }
    ...
    {
        if (isset($_GET['term'])) {
            $result = $this->cafe_model->cari_cust($_GET['term']);
            if (count($result) > 0) {
    ...
    {
        if (isset($_GET['term'])) {
            $result = $this->cafe_model->cari_sales($_GET['term']);
            if (count($result) > 0) {
                foreach ($result as $row) {
                    $arr_result[] = array(
                        'label' => $row->NmSales,
    ...
    {
        if (isset($_GET['term'])) {
            $result = $this->cafe_model->cari_brg($_GET['term']);
            if (count($result) > 0) {
                foreach ($result as $row) {
                    $arr_result[] = array(
                        'kode' => $row->KdBrg,
    ...
    public function go_to_simpan()
    {
        ...
        $nofaktur = $this->cafe_model->getNoFaktur($ymd, $id_user, $nm_user);
        $cek = $this->cafe_model->cek_go_simpan($noresi, $nm_user);
        $urix = base_url('cafe/form_cafe/') . $noresi;
        $uri = base_url('cafe/nomor-faktur/');
        $x = $cek->row_array();
        $cust = $x['KdCust'];
    ...
    public function cekcust()
    {
        ...
        $customer = $this->cafe_model->getcustomer($idcust);
        $cek_sudah_ada = $this->cafe_model->cek_cust_ada($idcust, $nofaktur);
    ...
    public function ceksales()
    {
        ...
        $sales = $this->cafe_model->getsales($idsales);
        $cek_sudah_ada = $this->cafe_model->cek_sales_ada($idsales, $nofaktur);
        $x = $sales->row_array();
        $kdsales = $x['KdSales'];
        //$nmsales = $x['NmSales'];
        $uri = base_url('cafe/form-cafe/') . $nofaktur;
    ...
    public function edit_satuan()
    {
        ...
        $cek_satuan_ada = $this->cafe_model->cek_satuan_ada($idbarang, $nofaktur);
    ...
    public function edit_jumlah_beli()
    {
        ...
        $cek_stok = $this->cafe_model->cek_jumlah_stok($idbarang);
        $rinci = $this->cafe_model->cek_sudah_ada($idbarang, $nofaktur);
    ...
    public function edit_kdsales()
    {
        ...
        $nofaktur = $this->input->post('nofak');
        $idbarang = $this->input->post('kdbrg');
        $ceksales = $this->cafe_model->ceksales_detail($idsales, $idbarang, $nofaktur);
        $getsales = $this->cafe_model->getsales($idsales);
    ...
    public function hapus_faktur()
    {
        ...
        redirect('cafe/nomor-faktur/', 'refresh');
    }
    ...
    public function cetak_struk()
    {
        ...
        $uri = base_url('cafe/form-cafe/') . $nofaktur;
        $this->db->trans_start();
        $data_faktur = $this->cafe_model->getPenjualanSelesai($nofaktur, $id_user)->row();
        $list_produk = $this->cafe_model->getProdukDijual($nofaktur)->result();
        if ($data_faktur && $list_produk) {
            foreach ($list_produk as $key) {
                $kd_barang_item = $key->kd_barang;
                $jumlah_item = $key->jumlah;
                $validasi_stok = $this->cafe_model->getStok($kd_barang_item);
                ...
                if ($stok_sekarang < $jumlah_item) {
                    ...
                    $stok_porsi = $this->cafe_model->getStokPorsi($kd_barang_item)->result();
            ...
            $data_cetak['toko'] = $this->cafe_model->get_toko();
    ...
    public function reprint_struk()
    {
        ...
        $data_faktur = $this->cafe_model->reprintStruk($nofaktur)->row();
        $produk = $this->cafe_model->getProdukDijual($nofaktur);
        if ($data_faktur) {
