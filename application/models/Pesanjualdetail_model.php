...
class Pesanjualdetail_model extends CI_Model
{

	public function daftarpesanjualdetail()
	{
		return $this->db->query("SELECT MP.* FROM mpesanjual AS MP INNER JOIN (SELECT NoPesanJual FROM tpesanjual GROUP BY NoPesanJual) AS TP ON MP.NoPesanJual=TP.NoPesanJual WHERE MP.NoPesanJual LIKE '%PJ%' ORDER BY MP.NoPesanJual DESC");
	}
	...
	public function ambildatadetail($nofaktur)
	{
		return $this->db->query("SELECT Jumlah,Harga,Disc_per,Disc FROM tpesanjual WHERE NoPesanJual = '$nofaktur'");
	}

	public function getDataPenjualan($noresi)
	{
		$this->db->where('NoPesanJual', $noresi);
		//$this->db->where('NmUser', $username);
		$this->db->where('Flag_SAVE', '1');
